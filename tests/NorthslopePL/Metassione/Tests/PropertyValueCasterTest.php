...
class PropertyValueCasterTest extends \PHPUnit_Framework_TestCase
{
	...
	{
		$integerProperty = new PropertyDefinition('integerNullable', true, $isObject = false, $isArray = true, PropertyDefinition::BASIC_TYPE_INTEGER, $isNullable = false, $this->reflectionProperty);

		$this->assertSame([], $this->propertyValueCaster->getBasicValueForProperty($integerProperty, 1));
		$this->assertSame([], $this->propertyValueCaster->getBasicValueForProperty($integerProperty, 0));
		//
		$this->assertSame([], $this->propertyValueCaster->getBasicValueForProperty($integerProperty, 'foobar'));
		$this->assertSame([], $this->propertyValueCaster->getBasicValueForProperty($integerProperty, ''));
		$this->assertSame([], $this->propertyValueCaster->getBasicValueForProperty($integerProperty, '123.456'));
		//
		$this->assertSame([], $this->propertyValueCaster->getBasicValueForProperty($integerProperty, 12.34));
		$this->assertSame([], $this->propertyValueCaster->getBasicValueForProperty($integerProperty, 0.0));
		//
		$this->assertSame([], $this->propertyValueCaster->getBasicValueForProperty($integerProperty, true));
		$this->assertSame([], $this->propertyValueCaster->getBasicValueForProperty($integerProperty, false));
		//
		$this->assertSame([], $this->propertyValueCaster->getBasicValueForProperty($integerProperty, new stdClass()));
		//
		$this->assertSame([], $this->propertyValueCaster->getBasicValueForProperty($integerProperty, []));
		$this->assertSame([1, 2, 0], $this->propertyValueCaster->getBasicValueForProperty($integerProperty, [1, 2, 0]));
		$this->assertSame([0, 0, 0], $this->propertyValueCaster->getBasicValueForProperty($integerProperty, ['aa', 'bb', '']));
		$this->assertSame([12, 56, 0], $this->propertyValueCaster->getBasicValueForProperty($integerProperty, [12.34, 56.78, 0.0]));
		$this->assertSame([1, 0], $this->propertyValueCaster->getBasicValueForProperty($integerProperty, [true, false]));
		$this->assertSame([], $this->propertyValueCaster->getBasicValueForProperty($integerProperty, [new stdClass(), new stdClass()]));
		$this->assertSame([], $this->propertyValueCaster->getBasicValueForProperty($integerProperty, [[1, 2], ['a', 'b'], [1.0, 2.0]])); // we dont handle this stuff
		//
		$this->assertSame([], $this->propertyValueCaster->getBasicValueForProperty($integerProperty, null));
		$this->assertNull($this->propertyValueCaster->getEmptyBasicValueForProperty($integerProperty));
	}
}
